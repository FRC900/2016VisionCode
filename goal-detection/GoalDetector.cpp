...
bool GoalDetector::processFrame(const Mat& image, const Mat& depth, Rect &bound)
{
    Mat threshold_image;
...
pair<float, float> GoalDetector::minOfMat(const Mat& img, const Mat& mask, bool (*f)(float), const Rect &bound_rect, int range)
{
	if (img.empty())
    ...
    return make_pair(sum_min / (num_pix_min * 1000.), sum_max / (num_pix_max * 1000.));
}
