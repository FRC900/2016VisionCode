...
for $d24_dir (sort @d24_model_dirs)
{
	...
	for $d12_dir (sort @d12_model_dirs)
	{
		...
		next if ($d12_num == -1);
		print "$c12_thresh, $d12_num, $d24_num, ";
		for $video (sort @videos)
		{
			open (my $pipeh, "./zv --c12Threshold=$c12_thresh --d12Dir=$d12_num --d24Dir=$d24_num --batch --groundTruth \"$video\"  2>/dev/null|");
			while ($line = <$pipeh>)
			{
